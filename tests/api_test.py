...
def test_get_recommended_routines(data: dict):
    ...
    assert response.json() == {
        'count': 3,
        'recommended_routines': data['routines']
    }
...
def test_get_routine(data: dict):
    ...
    assert response.json() == {
        'Error': 'Routine with id 3 not found.'}
...
def test_upload_routine(data: dict):
    ...
    routine = RoutineModel(
        id=3, workout_table=Routine.build_workout_table(new_routine_exercises), **new_routine_data)
...
def test_patch_routine(data: dict):
    ...
    response = client.patch(
        '/routines/{0}'.format(routine.id), json=data_to_edit)
